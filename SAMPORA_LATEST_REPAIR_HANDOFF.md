...
**Notes for next handoff:** `动态/index.html` is useful as a motion/reference artifact, but it is not safe as a direct public-source replacement without acceptance hardening. Future agents must verify any dynamic-file backfill before leaving it in `sampora-website-public-v3/`, especially canonical, CTA intent, legal/ICP, forbidden positioning copy, topology packet visibility, and inline script parsing.

- **Pitfall / symptom:** Directly copying `动态/index.html` into the public v3 homepage can make `index.html` fail static acceptance and inline script parsing.
