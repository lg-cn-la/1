...
async function sampleStickyNavAfterScroll(file) {
  ...
  await page.evaluate(() => {
    ...
    scrollTo(0, Math.min(maxY, Math.max(320, Math.floor(maxY * 0.45))));
  });
  ...
  const sample = await page.evaluate(() => {
    ...
    const statusOutOfTop = !statusRect || statusRect.bottom <= 1.5 || Number(statusStyle.opacity) <= 0.05;
    const mastAtTop = !!mastRect && mastRect.y >= -1.5 && mastRect.y <= 1.5;
    return {
  ...
  if (!sample.mastAtTop) {
    pushFailure(`${file}: sticky nav .mast is not pinned to viewport top after scroll: ${JSON.stringify(sample)}`);
  }
  if (!sample.statusOutOfTop) {
    pushFailure(`${file}: status strip still occupies viewport top after scroll: ${JSON.stringify(sample)}`);
  }
}
...
const scanOptionalTopologyPages = new Set(['index.html', 'solutions.html']);
for (const file of ['index.html', 'solutions.html', 'resources.html']) {
  ...
  const flowDashOffsets = evidence.topology[file].map(sample => sample.flowStrokeDashoffset).filter(Boolean);
  if (!latest.flowOnCount || latest.flowAnimationName === 'none') pushFailure(`${file}: topology flow animation not active`);
  if (new Set(flowDashOffsets).size < 2) pushFailure(`${file}: topology flow dash offset did not change`);
  if (!scanOptionalTopologyPages.has(file) && latest.scanAnimationName !== 'topoScan') pushFailure(`${file}: topo scan animation is ${latest.scanAnimationName}`);
