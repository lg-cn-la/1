...
```powershell
node "sampora-website-public\qa-evidence\final-audit-static-check.mjs"
node "sampora-website-public\qa-evidence\page-layer-static-check.mjs"
node --check "sampora-website-public\assets\i18n-sweep.js"
node --check "sampora-website-public\assets\resource-manuals\data.js"
node --check "sampora-website-public\assets\resource-manuals\app.js"
node "sampora-website-public\qa-evidence\resource-manuals-language-smoke.mjs"
node "sampora-website-public\qa-evidence\resource-manuals-browser-language-check.mjs"
node "sampora-website-public\qa-evidence\page-layer-browser-check.mjs"
node "sampora-website-public\qa-evidence\lighthouse-axe-check.mjs"
```

Zip acceptance after source checks pass:

- `sampora-website-public.zip` and `sampora-website-handoff.zip` must be rebuilt from current source.
...
- Handoff zip must contain current handoff docs and `redirect-map.md`.
...
|---|---|---|
| Scope | Only latest-audit or newest-user-message items are changed. Unmentioned content is out of scope. Only `[CONFIRMED]` items receive page/source repairs; `[VERIFY-FIRST]` is verification-only until proven; `[STRATEGY]` is docs/QA/packaging policy only. | `AGENTS.md` + handoff intake gate; read-only verification subagent reports for long lists; issue status table; reviewer must inspect scoped diffs. |
...
| Topology motion | Active flow has `animationName !== none`; `strokeDashoffset` changes over time; scan animation is active; active packets are visible. Do not validate motion by screenshots alone. | `page-layer-browser-check.mjs`; rerun after topology edits. |
...
| Navigation/sticky | Canonical pages keep consistent nav/header spacing. Plans page sticky nav remains visible after scrolling. | `page-layer-browser-check.mjs` records `evidence.sticky` for index, solutions, resources, resource-manuals, plans, and contact at 1440px; plans scrolls to `.matrix-wrap` when present and asserts `.mast` remains measurable and visible after scroll. |
...
6. Classify the issue with an issue status tag and include it in the issue status table.
7. Repair only `[CONFIRMED]` items in assigned package files. Use `[VERIFY-FIRST]` only for verification work and `[STRATEGY]` only for docs/QA/packaging policy.
8. Rerun the acceptance check and the relevant final controller checks.
9. Output scoped diff summary and fresh evidence.
10. Record the pitfall in `SAMPORA_LATEST_REPAIR_HANDOFF.md` or detailed history in `SAMPORA_LATEST_REPAIR_LOG.md`.
